...
-- Trains on DT_HTTP_ERROR_RATE (2-day baseline).
...
CREATE OR REPLACE SNOWFLAKE.ML.ANOMALY_DETECTION GRIZL.OBSERVABILITY.ROUTE_LATENCY_ANOMALY_MODEL(
  INPUT_DATA    => SYSTEM$REFERENCE('DYNAMIC TABLE', 'GRIZL.OBSERVABILITY.DT_ROUTE_LATENCY'),
  SERIES_COLNAME  => 'SERIES_KEY',
  TIMESTAMP_COLNAME => 'TIME_BIN',
...
CREATE OR REPLACE SNOWFLAKE.ML.ANOMALY_DETECTION GRIZL.OBSERVABILITY.ERROR_SIGNATURE_ANOMALY_MODEL(
  INPUT_DATA    => SYSTEM$REFERENCE('DYNAMIC TABLE', 'GRIZL.OBSERVABILITY.DT_ERROR_SIGNATURES'),
  SERIES_COLNAME  => 'SERIES_KEY',
  TIMESTAMP_COLNAME => 'TIME_BIN',
...
$$
DECLARE
  detect_ts TIMESTAMP_LTZ DEFAULT CURRENT_TIMESTAMP();
BEGIN
  -- Create a temporary view over the last 30 minutes for detection input
  CREATE OR REPLACE TEMPORARY VIEW GRIZL.OBSERVABILITY.TMP_ERROR_RATE_RECENT AS
  ...
  -- Run detection
  CREATE OR REPLACE TEMPORARY TABLE GRIZL.OBSERVABILITY.TMP_ERROR_RATE_RESULTS AS
  SELECT * FROM TABLE(
  ...
  FROM GRIZL.OBSERVABILITY.TMP_ERROR_RATE_RESULTS r
...
DECLARE
  detect_ts TIMESTAMP_LTZ DEFAULT CURRENT_TIMESTAMP();
BEGIN
  CREATE OR REPLACE TEMPORARY VIEW GRIZL.OBSERVABILITY.TMP_LATENCY_RECENT AS
  SELECT TIME_BIN, SERIES_KEY, SERVICE, ROUTE, P95_DURATION_MS
  FROM GRIZL.OBSERVABILITY.DT_ROUTE_LATENCY
  WHERE TIME_BIN >= DATEADD('minute', -30, CURRENT_TIMESTAMP());

  CREATE OR REPLACE TEMPORARY TABLE GRIZL.OBSERVABILITY.TMP_LATENCY_RESULTS AS
  SELECT * FROM TABLE(
    GRIZL.OBSERVABILITY.ROUTE_LATENCY_ANOMALY_MODEL!DETECT_ANOMALIES(
      INPUT_DATA      => SYSTEM$REFERENCE('VIEW', 'GRIZL.OBSERVABILITY.TMP_LATENCY_RECENT'),
      SERIES_COLNAME  => 'SERIES_KEY',
      TIMESTAMP_COLNAME => 'TIME_BIN',
      TARGET_COLNAME  => 'P95_DURATION_MS'
    )
  ...
  SELECT
    :detect_ts, 'ROUTE_LATENCY_ANOMALY_MODEL', r.TS, r.SERIES,
    SPLIT_PART(r.SERIES, '::', 1), SPLIT_PART(r.SERIES, '::', 2),
    r.Y, r.FORECAST, r.LOWER_BOUND, r.UPPER_BOUND,
    r.IS_ANOMALY, r.PERCENTILE, r.DISTANCE, 'route_latency_ml'
  FROM GRIZL.OBSERVABILITY.TMP_LATENCY_RESULTS r
  WHERE r.IS_ANOMALY = TRUE
    AND r.TS >= DATEADD('minute', -15, CURRENT_TIMESTAMP());
